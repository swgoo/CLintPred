...
from modules.dataloader import BiomakerDataModule, clearanceKfoldDatamodule
...
def train_function(config, logger):
    ...
    ## -- datamodule -- ##
    datamodule = clearanceKfoldDatamodule(chem_tokenizer, config, 
                                        df_trainData, df_testData,
                                        train_labels, test_labels,
                                        train_scaler, test_scaler,
                                        df_trainfeatures, df_testfeatures, args.kfold)
    ...
    for fold in range(args.kfold):
        ...
        trainer.fit(clearance_model, datamodule.train_dataloader(fold_index=fold), datamodule.val_dataloader())

        clearance_model.eval()
        trainer.test(clearance_model, datamodule.test_dataloader())
        ...
        outputs_list = list()
        valid_log, test_log = clearance_model.valid_log, clearance_model.test_log
        ...
        df_metricData.loc[len(df_metricData)] = [config.num_seed, config.model_type, config.chem_model, config.checkpoint_name, config.feature_type, config.test_rate, \
                                                config.augmentation, config.extend_protType, config.scale, \
        ...
        resultData_path = f"{path_name}_kfold/"
        evaluation_path = f"./results/{resultData_path}" + f"fold_{fold}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", help="config_file", type=str, default="1_config_kfold.json")
    parser.add_argument("--kfold", help="kfold setting", type=int, default=5)
    ...
    if run_type == SWEEP:
        config = load_hparams('config/sweep/1_config_kfold.json')
        project_name = config["name"]
        sweep_id = wandb.sweep(config, project=project_name)
        wandb.agent(sweep_id, main)
    
    else:
        config_file = f'config/{args.config}' if run_type == TENSOR_SWEEP else 'config/1_config_kfold.json'
